...
public class demo1 {
		...
		public static void main(String[] args) throws InterruptedException
		{
			...
			WebElement act = driver.findElement(By.xpath("//input[@class='new-search-inp']"));
			builder.moveToElement(act).build().perform();
			Thread.sleep(3000);
			
			WebElement search = driver.findElement(By.xpath("//i[@class='fa fa-search search_ico trackButton']"));
			builder.moveToElement(search).build().perform();
			Thread.sleep(3000);
			
			Action seriesOfActions;
			seriesOfActions = builder.sendKeys(act, "Selenium").keyDown(search, Keys.SHIFT).keyUp(search, Keys.SHIFT).build();
